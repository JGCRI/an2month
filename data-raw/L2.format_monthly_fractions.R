...
format_data <- function(input_path, var, output_dir){
  ...
  outfile <- file.path(output_dir, gsub(basename(input_path), pattern = '.nc', replacement = '.rds'))
  if(!file.exists(outfile)){
    ...
    # Subset the data so that it only includes the results for the complete years,
    # any extra months are dropped.
    t <- (12*nyears)
    time <- time[1:t]
    data <- data_og[, ,1:t]
